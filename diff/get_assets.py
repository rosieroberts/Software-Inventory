...
class getAssets:
    """ Class that converts command line arguments,
        hostname, club, asset_tag and licenses and creates a list of dictionaries
        with the asset information stored in mongo that is associated with the argument
        """
    def __init__(self):
        self.asset_list_hw = []
        self.asset_list_sw = []
    ...
    def get_asset_list(self, asset_list):
        ...
        if not asset_list:
            ...
            self.asset_list_hw = snipe_hw.find({}).sort('Asset Tag', pymongo.ASCENDING)
        ...
        for item in asset_list:
            ...
            if club:
                assets = snipe_hw.find({'Location': item})
            elif asset_tag:
                assets = snipe_hw.find({'Asset Tag': item})
            elif hostname:
                assets = snipe_hw.find({'Hostname': item})
            else:
            ...
            if assets:
                ...
                if asset_tag:
                    ...
                    continue
        # only found assets and deleted assets associated with a seat will be returned
        # This message for review.
    ...
    def get_lic_list(self, lic_list):
        ''' Only runs when a list of licenses ('123' format) is provided in arguments
        ...
        for item in lic_list:
            ...
            if not lic:
                continue
            # make sure the input matches the license number regex and looks only for license with
            # active assets seats with no assets associated with them will have a None in
            # asset_name in the snipe_seats collection
            ...
            for seat in lic_item:
                asset = snipe_hw.find_one({'Location': seat['location'],
                                           'Hostname': seat['asset_name']})
                if asset:
                    # if there is a record of a an asset associated with this license, append the
                    # asset info to list
                    self.asset_list_sw.append(asset)
                    found_assets_ct += 1
                else:
                    # if an asset has been deleted the asset info will be in the deleted collection
                    # in the 'inventory' mongodb if there is a case where the asset was deleted but
                    # the seat is still checked out, this will return the asset info
                    if seat['asset_name']:
                        ...
                        if del_asset:
                            ...
                            not_found_assets_ct += 1
                            logger.debug('error, there is a seat associated to an asset not found.'
                                         'Review lic {} seat {}'
                                         .format(item, seat['id']))
                            continue
                    else:
